...
class Minesweeper():
    ...
    def __init__(self, height=10, width=10, mines=10):# no mines
    ...
    def random_mines(self, height=8, width=8, mines=8,x_firstmov=0,y_firstmove =0): # add mines randomly after first move
        # Add mines randomly
        while len(self.mines) != mines:
            i = random.randrange(height)
            j = random.randrange(width)
            if not self.board[i][j] and not abs(x_firstmov-i)<=1 and not abs(y_firstmove-j)<=1:
                self.mines.add((i, j))
                self.board[i][j] = True
